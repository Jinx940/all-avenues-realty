type FinanceWorkbookValueFormat = 'text' | 'integer' | 'currency' | 'percent';

type FinanceWorkbookMetaRow = {
  label: string;
  value: string | number;
  format?: FinanceWorkbookValueFormat;
};

type FinanceWorkbookMetricRow = {
  metric: string;
  value: number;
  format: Exclude<FinanceWorkbookValueFormat, 'text'>;
};

type FinanceWorkbookSection = {
  title: string;
...
const downloadBuffer = (fileName: string, buffer: ArrayBuffer) => {
...
export const downloadFinanceWorkbook = async (input: FinanceWorkbookInput) => {
  ...
  downloadBuffer(input.fileName, buffer as ArrayBuffer);
};
