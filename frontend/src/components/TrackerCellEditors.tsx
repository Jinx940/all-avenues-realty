...
export function TrackerPopover({ anchor, title, compact = false, onClose, children }: {
  ...
  return createPortal(<div ref={ref} className={`jt-popover${compact ? ' jt-popover--summary' : ''}`} role="dialog" aria-label={compact ? title : undefined} aria-labelledby={compact ? undefined : titleId}>
    {compact ? null : <div className="jt-popover-title"><strong id={titleId}>{title}</strong><button type="button" aria-label="Close menu" onClick={onClose}><UiIcon name="close" size={15} /></button></div>}
    {children}
  </div>, document.body);
}
...
export function TrackerLabelCell({ kind, job, labels, canManage, onUpdate, onLabelsChange }: {
  ...
  const title = kind === 'status' ? 'Status' : 'Priority';
  ...
  return <td className="jt-status jt-editable-label" style={labelStyle(selected)}>
    {canManage ? <button type="button" aria-label={`Change ${title.toLowerCase()}: ${job.service}`} aria-haspopup="dialog" aria-expanded={Boolean(anchor)}
      onClick={(event) => { setAnchor(event.currentTarget); setError(''); }}>
      {selected.label}
    </button> : <span>{selected.label}</span>}
    {anchor ? <TrackerPopover anchor={anchor} title={editing ? `Edit labels for ${title.toLowerCase()}` : title} onClose={close}>
      {editing ? <form className="jt-label-form" onSubmit={(event) => { event.preventDefault(); void save(() => onLabelsChange(draft)); }}>
        <p>Names and colors apply to every job on the board.</p>
        {draft.map((label, index) => <div className="jt-label-edit-row" key={label.value}>
          <input type="color" value={label.color} aria-label={`Color for ${label.value}`} disabled={saving}
            onChange={(event) => setDraft((items) => items.map((item, position) => position === index ? { ...item, color: event.target.value } : item))} />
          <input value={label.label} required maxLength={40} aria-label={`Name for ${label.value}`} disabled={saving}
            onChange={(event) => setDraft((items) => items.map((item, position) => position === index ? { ...item, label: event.target.value } : item))} />
        </div>)}
        <div className="jt-editor-actions"><button type="button" onClick={() => setEditing(false)} disabled={saving}>Back</button><button type="submit" disabled={saving}>{saving ? 'Saving…' : 'Save labels'}</button></div>
      </form> : <>
        ...
        <button type="button" className="jt-edit-labels" disabled={saving} onClick={() => { setDraft(options.map((label) => ({ ...label }))); setEditing(true); }}><UiIcon name="file" size={15} />Edit labels</button>
      </>}
...
export function TrackerTimelineCell({ job, canManage, onUpdate }: {
  ...
  const [anchor, setAnchor] = useState<HTMLElement | null>(null);
  const short = (date: string | null) => date ? new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' }).format(new Date(date)) : 'No date';
  const text = !job.startDate && !job.dueDate ? 'No dates' : `${short(job.startDate)} – ${short(job.dueDate)}`;
  ...
  return <td className={`jt-data-cell jt-timeline-cell${days > 0 ? ' jt-has-duration' : ''}`} title={days > 0 ? `${days} ${days === 1 ? 'day' : 'days'}` : text}>
    {canManage ? <button type="button" className="jt-cell-button" aria-haspopup="dialog" aria-expanded={Boolean(anchor)} aria-label={`Edit timeline: ${job.service}`} onClick={(event) => setAnchor(event.currentTarget)}>{content}</button>
      : content}
...
function TrackerDateEditor({ anchor, job, onClose, onUpdate }: {
  ...
  return <TrackerPopover anchor={anchor} title="Set dates" onClose={close}>
    <form className="jt-date-editor" onSubmit={(event) => { event.preventDefault(); void save(); }}>
      <div className="jt-date-fields">
        <label>Start<input type="date" aria-label="Start date" value={start} disabled={saving} onFocus={() => setSelecting('start')}
          onChange={(event) => { setStart(event.target.value); if (event.target.value) setMonth(event.target.value.slice(0, 7)); }} /></label>
        <label>End<input type="date" aria-label="End date" value={end} disabled={saving} onFocus={() => setSelecting('end')} onChange={(event) => setEnd(event.target.value)} /></label>
      </div>
      <div className="jt-calendar-nav"><input type="month" value={month} aria-label="Calendar month" disabled={saving} onChange={(event) => { if (event.target.value) setMonth(event.target.value); }} />
        <button type="button" aria-label="Previous month" disabled={saving} onClick={() => changeMonth(-1)}>‹</button><button type="button" aria-label="Next month" disabled={saving} onClick={() => changeMonth(1)}>›</button></div>
      <div className="jt-calendar-grid" role="group" aria-label="Select date range">
      ...
      <p className="jt-calendar-hint" role="status">{count ? `${count} ${count === 1 ? 'day selected' : 'days selected'}` : selecting === 'end' ? 'Select the end date' : 'Select the start date'}</p>
      {invalid ? <p role="alert" className="jt-editor-error">The end date must be on or after the start date.</p> : null}
      {error ? <p role="alert" className="jt-editor-error">{error}</p> : null}
      <div className="jt-editor-actions"><button type="button" disabled={saving} onClick={() => { setStart(''); setEnd(''); setSelecting('start'); }}>Clear dates</button>
        <button type="submit" disabled={saving || invalid}>{saving ? 'Saving…' : 'Save dates'}</button></div>
    </form>
  </TrackerPopover>;
}
...
export function TrackerSummaryCell({ jobs, labels, kind, propertyName }: { jobs: JobRow[]; labels: TrackerLabel[]; kind: TrackerLabel['kind']; propertyName: string }) {
  ...
  const title = `${kind === 'status' ? 'Status' : 'Priority'} summary for ${propertyName}`;
